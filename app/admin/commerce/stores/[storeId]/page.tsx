...
export default async function AdminStoreDetailPage({ params, searchParams }: { params: Promise<{ storeId: string }>; searchParams: Promise<{ auditCursor?: string }> }) {
  const { storeId } = await params;
  const query = await searchParams;
  const context = await requireAuthenticatedCommerceAdmin("COMMERCE_STORES_VIEW");
  const store = await getAdminStoreDetail(context, storeId, { auditCursor: query.auditCursor });
  return <>
