...
export async function POST(request: Request, route: { params: Promise<{ jobId: string }> }) {
  ...
  return handlePlatformJobAdminRequest("jobs.requeue", "PLATFORM_JOBS_MANAGE", async (context) =>
    requeuePlatformJob(context, { ...parsePlatformJobVersionedMutation(await readBoundedPlatformJobJson(request)), jobId }));
}
