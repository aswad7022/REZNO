...
export async function GET(request: NextRequest, context: { params: Promise<{ assetId: string }> }) {
  ...
  return handleStorageRequest(request, "business", "asset.detail", (actor) =>
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ assetId: string }> }) {
  const { assetId } = await context.params;
  return handleStorageRequest(request, "business", "asset.delete", async (actor) =>
    deleteStoredAsset(actor, { ...await parseVersionMutation(request), assetId: routeUuid(assetId, "assetId") }), 20);
}
