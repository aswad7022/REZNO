...
export async function POST(request: NextRequest, context: { params: Promise<{ sessionId: string }> }) {
  ...
  return handleStorageRequest(request, "business", "session.abort", async (actor) =>
    abortUpload(actor, { ...await parseVersionMutation(request), sessionId: routeUuid(sessionId, "sessionId") }), 30);
}
