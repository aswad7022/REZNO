...
export async function POST(request: NextRequest, context: { params: Promise<{ sessionId: string }> }) {
  ...
  return handleStorageRequest(request, "customer", "target.issue", async (actor) =>
    issueUploadTarget(actor, { ...await parseVersionMutation(request), sessionId: routeUuid(sessionId, "sessionId") }), 20);
}
