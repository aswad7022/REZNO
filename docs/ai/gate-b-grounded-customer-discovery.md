...
- Gemini receives only the sanitized question and a compact public result list.
- REZNO server validates provider JSON and citations, then creates links from trusted internal IDs.
...
Gate B refuses locally before Gemini when input contains emails, phone-like values, secrets, sessions, bookings, orders, payments, admin/staff/platform requests, or prompt-injection attempts. It does not send customer profiles, bookings, orders, payments, messages, cookies, sessions, tokens, or private addresses to Gemini.
