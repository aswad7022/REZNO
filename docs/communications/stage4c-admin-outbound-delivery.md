...
Communication reporting cursors use authenticated envelope version 2. The server-only signer validates the existing `BETTER_AUTH_SECRET` security contract, derives a dedicated 32-byte key with HKDF-SHA-256 and the exact versioned info value `rezno:communications:cursor-signing:v2`, and authenticates one fixed-order canonical JSON representation with HMAC-SHA-256. Verification rejects malformed or non-64-hex MACs before fixed-length `timingSafeEqual`; every failure is exposed only as `INVALID_CURSOR`. The MAC binds version, kind, current Admin scope, filter fingerprint, page size, parent resource, fixed snapshot, sort timestamp, and UUID tie-breaker. Version 1 public-checksum cursors are invalidated without fallback or downgrade. Rotating `BETTER_AUTH_SECRET` invalidates outstanding pagination cursors; this is accepted for transient navigation state, and Gate 4C does not add a multi-key cursor rotation ring.
