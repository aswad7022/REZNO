...
Communication reporting cursors use authenticated envelope version 3. The server-only signer validates the existing `BETTER_AUTH_SECRET` security contract, derives a dedicated 32-byte key with HKDF-SHA-256 and the exact versioned info value `rezno:communications:cursor-signing:v3`, and authenticates one fixed-order canonical JSON representation with HMAC-SHA-256. Verification rejects malformed or non-64-hex MACs before fixed-length `timingSafeEqual`; every failure is exposed only as `INVALID_CURSOR`. The MAC binds version, kind, current Admin scope, filter fingerprint, page size, parent resource, fixed snapshot, sort timestamp, and UUID tie-breaker. Snapshot and sort timestamps preserve exact PostgreSQL microseconds as `YYYY-MM-DDTHH:mm:ss.ffffffZ`; page membership uses direct indexed `(createdAt, id)` tuple predicates before bounded Prisma hydration. Versions 1 and 2 are invalidated without fallback or downgrade. Rotating `BETTER_AUTH_SECRET` invalidates outstanding pagination cursors; this is accepted for transient navigation state, and Gate 4C does not add a multi-key cursor rotation ring.
