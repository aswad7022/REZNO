...
`npm run test:platform-jobs:unit` covers architecture boundaries, closed registry, strict payload/result schemas, private-data rejection, canonical hashing, lifecycle, lease/horizon, retry/backoff/jitter, schedule catch-up, signed microsecond cursors, body framing, safe exceptions, production test guards, Migrations 43–44, and fail-closed staging identity/TLS safety. Positive transport cases cover authorized client TLS with backend `pg_stat_ssl` both true and false. Negative cases cover unauthorized certificates, hostname mismatch, absent or expired peer certificates, plaintext Node sockets, wrong SNI/database/role, pooler and loopback endpoints, explicit `rejectUnauthorized=false`, malformed discovery, different probe/Prisma configuration, different physical backend, unhealthy migrations, and failure before mutation. Exact verified direct Neon and exact loopback test targets are the only positives.
...
Real staging must begin healthy 43/43 with Migration 44 absent. Authenticated discovery must provide the exact direct non-pooler host and expected role; the URL must use `verify-full`, URL/current roles must match, and the shared Pool must pass the full client-side TLS/certificate/hostname/identity and exact Prisma-backend reuse proof. `pg_stat_ssl` is same-client diagnostic evidence, not sole TLS authority. Deploy canonically to 44/44 and show a second deploy no-op. Seed twice must return an identical fixture fingerprint. Smoke must additionally prove worker-operation crashes before/after claim, active non-steal, expired recovery, canonical-attempt finalization, no batch expansion/duplicate execution, changed replay conflict, revoked Admin rejection, TLS identity, and provider/runtime truth. Cleanup must be exact, a second cleanup must remove zero, and the non-fixture database fingerprint must remain unchanged.
...
- focused unit 21/21 and complete unit 427/427;
...
The initial dependency inventory reported eight package findings: three High and five Moderate, with no Critical; Mobile reported zero. Remediation upgrades Sharp to 0.35.3, applies patched Fast-URI 3.1.4 and `@prisma/dev` 0.24.14 overrides, retains the known-compatible Prisma runtime at 7.8, and moves shadcn to development scope. A Prisma 7.9 trial was not required: its apparent test failures were traced to a missing test-only `BETTER_AUTH_SECRET`, not to Prisma, and the narrow 7.8-compatible leaf remediation closes the advisory without an unrelated runtime upgrade. The post-remediation production audit is 0/0/0/0. The full audit contains only three Moderate development-only shadcn/MCP/Hono findings, accepted as P3 after import, route, server bundle, browser bundle, and Mobile bundle reachability all returned false. The exact inventory is in `gate6a-dependency-advisory-review.md` and the sanitized machine evidence is in `evidence/gate6a-dependency-audit-evidence.json`.

The live transport preflight passed Probe A and Probe B at healthy 43/43: TLS 1.3, authorized peer, valid certificate, successful hostname/SNI verification, direct port, non-loopback route, matching database/role, and the same physical backend through Prisma. Backend `pg_stat_ssl=false` is retained as Neon proxy-boundary diagnostic evidence. Local psql/libpq was unavailable and is recorded as such rather than treated as proof.

The authenticated staging closure then applied only Migration 44 and reached healthy 44/44 with zero failed or rolled-back migrations. The second canonical deploy reported 44 migrations found and no pending migrations. Both seed runs produced the same fixture fingerprint `260f58ac3c8fbbf9a2f62e68806a33c9a8dc9dd6c585fdf12d9c099f66816d17`; the staging smoke passed 59/59 and preserved the non-fixture fingerprint `51f91a54f3d34335477ad613342c374803a26d6b401271973f7cffa89613d2d2`. Exact cleanup removed 35 scoped rows, the second cleanup removed zero, and the final 44/44 postflight retained that original non-fixture fingerprint.
