...
| Logout clears authentication while payment work continues | Logout first deactivates and awaits the exact active runner, including an old-owner runner already quiescing during an account switch. Prepare, browser, and verification phases are aborted where possible; a failed logout bootstraps the same owner again. A stale owner cannot deactivate the authoritative owner's runner. |
