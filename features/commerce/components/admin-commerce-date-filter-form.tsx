...
export function AdminCommerceDateFilterForm({
  children,
  className,
  dateFilters,
}: {
  children: ReactNode;
  className: string;
  dateFilters: readonly AdminDateFilter[];
}) {
  ...
  useEffect(() => {
    ...
    for (const filter of dateFilters) {
      const visible = form.querySelector<HTMLInputElement>(`[data-admin-date-local="${filter.name}"]`);
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    for (const filter of dateFilters) {
      const visible = event.currentTarget.querySelector<HTMLInputElement>(`[data-admin-date-local="${filter.name}"]`);
      ...
      if (!converted) {
        ...
        visible.setCustomValidity("أدخل تاريخًا ووقتًا صالحين.");
  ...
  return <form className={className} method="get" onSubmit={submit} ref={formRef}>
    {children}
    {dateFilters.map((filter) => <label className="grid gap-1 text-sm" key={filter.name}>
      <span>{filter.label}</span>
      <Input
        data-admin-date-local={filter.name}
