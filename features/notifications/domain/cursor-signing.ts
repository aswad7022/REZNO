...
export const NOTIFICATION_CURSOR_SIGNING_INFO =
  "rezno:notifications:cursor-signing:v2";
export const NOTIFICATION_CURSOR_MAC_BYTES = 32;
...
export function setNotificationCursorSigningSecretForTests(
  secret: string | null | undefined,
) {
  if (process.env.NODE_ENV === "production") {
    throw new Error("Notification cursor signing test configuration is unavailable.");
  }
  testSigningSecret = secret;
}

export function signNotificationCursor(canonicalInput: string): Buffer {
  return createHmac("sha256", resolveSigningKey())
    .update(canonicalInput, "utf8")
    .digest();
}

export function verifyNotificationCursorMac(
  canonicalInput: string,
  receivedMac: string,
) {
  if (!/^[a-f0-9]{64}$/.test(receivedMac)) return false;
  const received = Buffer.from(receivedMac, "hex");
  const expected = signNotificationCursor(canonicalInput);
  if (
    received.length !== NOTIFICATION_CURSOR_MAC_BYTES
    || expected.length !== NOTIFICATION_CURSOR_MAC_BYTES
...
function resolveSigningKey() {
  ...
  return Buffer.from(hkdfSync(
    ...
    Buffer.alloc(0),
    Buffer.from(NOTIFICATION_CURSOR_SIGNING_INFO, "utf8"),
    NOTIFICATION_CURSOR_MAC_BYTES,
  ));
}
