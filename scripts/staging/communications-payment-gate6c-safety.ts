...
export async function assertCommunicationsPaymentGate6cStaging(
  ...
  const localUnencrypted = isExactLocalTestOverride(
    environment,
    target,
    connection,
  );
  if (!localUnencrypted) {
    assertExactRealStagingTarget(environment, target, connection);
  }
  ...
  return {
    backendPgStatSsl: localUnencrypted
      ? connection.encrypted
      : transportEvidence!.backendPgStatSsl,
    clientTlsVerified: localUnencrypted
      ? false
      : transportEvidence!.clientTlsVerified,
    database: "rezno_staging" as const,
    encrypted: localUnencrypted
      ? connection.encrypted
      : transportEvidence!.encrypted,
    ...
    transport: localUnencrypted
      ? "LOCAL_TEST_TCP" as const
...
function isExactLocalTestTarget(
  environment: NodeJS.ProcessEnv,
  target: DatabaseTarget,
) {
