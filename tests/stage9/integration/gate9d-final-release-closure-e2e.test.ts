...
async function assertDisposableDatabase() {
  if (process.env.NODE_ENV === "production") {
    throw new Error("Gate 9D integration refuses NODE_ENV=production.");
  }
  ...
  assert.match(
    rows[0]?.database ?? "",
    /(?:_test|test_|gate9d|gate9c|gate9b|gate9a|marketplace_ci)/,
    "Gate 9D integration requires a disposable PostgreSQL database.",
  );
}

test("Gate 9D reads the final release database baseline without mutating PostgreSQL", async () => {
  ...
  assert.deepEqual({
    ...
    applied: GATE9D_EXPECTED_MIGRATION_COUNT,
    failed: 0,
    rolledBack: 0,
    total: GATE9D_EXPECTED_MIGRATION_COUNT,
  });
  ...
  const migration52 = await prisma.$queryRaw<Array<{ migration_name: string }>>`
    SELECT migration_name
    FROM "_prisma_migrations"
    WHERE migration_name LIKE '2026%migration_52%'
       OR migration_name LIKE '%migration52%'
       OR migration_name LIKE '%gate9d%'
  `;
  assert.deepEqual(migration52, []);
